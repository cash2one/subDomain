...
def nmapScan(targetHosts,targetport,domain,taskid):
    """
    主要用来工作区域
    获取当前的ip地址加入Nmap扫描中
    如果发现地址存活，就输出服务等信息
    ...
    try:
        ...
        scanner.scan(targetHosts,arguments='-p T:'+str(targetport)+' -Pn -sT -sV --script=banner --allports --version-all --min-parallelism 100')
        for targetHost in scanner.all_hosts():
            if scanner[targetHost].state() == 'up' and scanner[targetHost]['tcp']:
                for targetport in scanner[targetHost]['tcp']:
                    if scanner[targetHost]['tcp'][int(targetport)]['state'] == 'open' or scanner[targetHost]['tcp'][int(targetport)]['product'].find('tcpwrapped')!=-1:
                        ...
                        db = MySQL(dbconfig)
                        sqlSearch = "select service from result_ports where taskid= '%s' and domain = '%s' and address= '%s' and port = '%s'" % (taskid,domain,str(targetHosts),str(targetport))
                        if int(db.query(sql=sqlSearch)) > 0:
                            result = db.fetchAllRows()
                            for row in result:
                                cmps = cmp(scanner[targetHost]['tcp'][int(targetport)]['name'],row[0])
                                if int(cmps) == 0:
                                    continue
                                else:
                                    sqlInsert = "insert into result_ports(id,taskid,domain,address,port,service,product,product_version,time) values ('','%s','%s','%s','%s','%s','%s','%s','%s')" % (taskid,domain,str(targetHosts),str(targetport),scanner[targetHost]['tcp'][int(targetport)]['name'],scanner[targetHost]['tcp'][int(targetport)]['product'],scanner[targetHost]['tcp'][int(targetport)]['version'],timestamp())
                                    db.insert(sql=sqlInsert)
                        else:
                            sqlInsert = "insert into result_ports(id,taskid,domain,address,port,service,product,product_version,time) values ('','%s','%s','%s','%s','%s','%s','%s','%s')" % (taskid,domain,str(targetHosts),str(targetport),scanner[targetHost]['tcp'][int(targetport)]['name'],scanner[targetHost]['tcp'][int(targetport)]['product'],scanner[targetHost]['tcp'][int(targetport)]['version'],timestamp())
                            #logging.info(sqlInsert)
                            db.insert(sql=sqlInsert)
                        resuls.append(({
                                "port": str(targetport),
...
def PortScanner(ip,num_of_threads,taskid,domain):
    if is_intranet(ip):
        sys.exit()
    _st = time.time()
    ...
    for port in ports:
        wm.add_job(nmapScan,str(ip),str(port),domain,taskid)
    wm.start()
...
def init_parser():
    ...
    parser.add_option("--host", type="str", dest="host", help="remote host IP")
    parser.add_option("--domain", type="str", dest="domain", default=None,help="chose domain for host")
    parser.add_option("--number", type="int", dest="num_of_threads",default=10, help="Number of threads. default = 10")
    parser.add_option("--taskid", type="str", dest="taskid", default=None, help="taskid")
    return parser
...
if __name__ == "__main__":
    ...
    num_of_threads = option.num_of_threads
    ...
    if domain == None:
        ...
        domain = domain
    print PortScanner(ip,num_of_threads,taskid,domain)
